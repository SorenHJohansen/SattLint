...
from .variable_utils import matches_root_origin, same_origin_file_stem
...
def _reset_path_children_of(obj: Any) -> list[Any] | None:
...
def is_from_root_origin(origin_file: str | None, root_origin: str | None) -> bool:
    return matches_root_origin(origin_file, root_origin)
    ...
    def visit(obj: Any) -> None:
        ...
        if isinstance(obj, tuple):
            for item in cast(tuple[Any, ...], obj):
                visit(item)
            return
        children = _reset_path_children_of(obj)
...
def _collect_reset_old_vars(modulecode: ModuleCode, reset_ref_cf: str) -> set[str]:
    ...
    def visit(obj: Any) -> None:
        ...
        if hasattr(obj, "data") and obj.data == const.KEY_STATEMENT:
            for child in _reset_path_children_of(obj) or []:
                visit(child)
            return
