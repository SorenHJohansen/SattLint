"""Variable-analysis selection catalog.

Maps interactive menu selection keys to ``IssueKind`` sets and groups them
into confidence tiers. This is pure analysis-domain data consumed by the
variable-analysis menu and the analyzer execution path.
"""

from __future__ import annotations

from .variables import IssueKind
...
VARIABLE_ANALYSES: VariableAnalysisMap = {
    ...
    "11": ("Sorting-sensitive datatypes", {IssueKind.RECORD_COMPONENT_ORDER_DEPENDENCE}),
    "12": ("Reset contamination", {IssueKind.RESET_CONTAMINATION}),
    "13": ("Variable shadowing", {IssueKind.SHADOWING}),
    "14": ("Unsafe boolean defaults", {IssueKind.UNSAFE_BOOLEAN_DEFAULT}),
    "15": ("Procedure status handling", {IssueKind.PROCEDURE_STATUS}),
    "16": ("Write-without-effect variables", {IssueKind.WRITE_WITHOUT_EFFECT}),
    "17": ("Implicit latching", {IssueKind.IMPLICIT_LATCH}),
    "18": ("Global scope minimization", {IssueKind.GLOBAL_SCOPE_MINIMIZATION}),
    "19": ("Hidden global coupling", {IssueKind.HIDDEN_GLOBAL_COUPLING}),
    "20": ("High fan-in or fan-out variables", {IssueKind.HIGH_FAN_IN_OUT}),
    "21": ("Read-only fields", {IssueKind.FIELD_READ_ONLY}),
    "22": ("Written but never read fields", {IssueKind.FIELD_NEVER_READ}),
}
