...
class EffectFlowTracker:
    ...
    def record_function_call_effect_flow(
        ...
        if array_spec is not None:
            if len(args) <= max(array_spec.array_index, array_spec.element_index):
                return
            array_ref = _var_ref_text(cast(object, args[array_spec.array_index]))
            element_ref = _var_ref_text(cast(object, args[array_spec.element_index]))
            if array_ref is None or element_ref is None:
                return
            if array_spec.effect_source == "element":
                source_key = self.resolve_effect_key(element_ref, context)
                target_key = self.resolve_effect_key(array_ref, context)
            else:
                source_key = self.resolve_effect_key(array_ref, context)
                target_key = self.resolve_effect_key(element_ref, context)
            self.record_effect_flow(source_key, target_key)
            return
    ...
    def propagate_mapping_to_parent(  # noqa: PLR0915
        self,
        pm: ParameterMapping,
