...
VARIABLE_RULES: dict[IssueKind, SemanticRule] = {
    ...
    IssueKind.NAMING_ROLE_MISMATCH: SemanticRule(
        ...
        name="Naming role mismatch",
        example='LOCALVARIABLES\n   CmdRegister: integer := 0;  (* "Cmd" name suggests it is written *)\nModuleCode\n   DisplayValue = CmdRegister + 1;  (* only read, never written *)',
    ),
    IssueKind.UI_ONLY: SemanticRule(
        ...
        name="UI-only variable",
        example="LOCALVARIABLES\n   DisplayValue: integer := 0;\nModuleCode\n   DisplayValue = RawInput;  (* value only shown on a faceplate *)",
    ),
    ...
    IssueKind.REQUIRED_PARAMETER_CONNECTION: SemanticRule(
        ...
        name="Required parameter not connected",
        example="TYPEDEFINITIONS\n   ValveType = MODULEDEFINITION DateCode_ 1\n   MODULEPARAMETERS\n      OpenCmd: boolean;\n   ModuleDef\n   ENDDEF (*ValveType*);\nSUBMODULES\n   V001 Invocation\n      ( 0.0, 0.0, 0.0, 0.3, 0.3 ) : ValveType ();  (* OpenCmd left unconnected *)",
    ),
    ...
    IssueKind.NAME_COLLISION: SemanticRule(
        ...
        name="Name collision",
        example="LOCALVARIABLES\n   PumpSpeed: integer := 0;\n   pumpspeed: integer := 0;  (* same name in a case-insensitive scope *)",
    ),
...
SFC_RULES: dict[str, SemanticRule] = {
    ...
    "sfc_transition_always_true": SemanticRule(
        ...
        name="Transition always true",
        example="SEQTRANSITION TrAuto WAIT_FOR Level <= Level  (* always true when reached *)\n   (* the guard carries no real condition *)",
    ),
    "sfc_transition_always_false": SemanticRule(
        ...
        name="Transition always false",
        example="SEQTRANSITION TrGo WAIT_FOR Level < Level  (* can never be true *)",
    ),
    ...
    "sfc_illegal_state_combination": SemanticRule(
        ...
        name="Illegal state combination",
        example="SEQUENCE SafetySeq (SeqControl, SeqTimer) COORD 0.0, 0.0 OBJSIZE 1.0, 1.0\n   SEQSTEP Extend            (* must never be active with Retract *)\n   SEQSTEP Retract",
    ),
    "sfc_missing_step_enter_contract": SemanticRule(
        ...
        name="Missing step enter write",
        example="SEQSTEP Run\n   ACTIVECODE\n      Output = Level;   (* reads StepValue ... *)\n   (* no ENTERCODE write initializes StepValue before it is read *)",
    ),
    "sfc_missing_step_exit_contract": SemanticRule(
        ...
        name="Missing step exit write",
        example="SEQSTEP Stop\n   (* no EXITCODE clears Mutex variable when the step ends *)\nSEQTRANSITION TrDone WAIT_FOR True",
    ),
    "sfc_step_state_leakage": SemanticRule(
        ...
        example="SEQSTEP Prime\n   ACTIVECODE\n      StepValue = 1;\nSEQSTEP Run\n   ACTIVECODE\n      Output = StepValue;  (* runs with the value Prime left behind *)",
    ),
}
...
ALARM_RULES: dict[str, SemanticRule] = {
    ...
    "alarm.conflicting_priority": SemanticRule(
        ...
        name="Conflicting alarm priority",
        example='SUBMODULES\n   Alarm1 Invocation\n      ( 0.0, 0.0, 0.0, 0.4, 0.4 ) : EventDetector1 (Tag => "PRESS_HIGH", Priority => 1);\n   Alarm2 Invocation  (* same tag, different priority *)\n      ( 0.5, 0.0, 0.0, 0.4, 0.4 ) : EventDetector1 (Tag => "PRESS_HIGH", Priority => 2);',
    ),
