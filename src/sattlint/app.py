...
from .application import commands as app_analysis_commands_module
...
from .project import support as project_support_module
...
VARIABLE_ANALYSES: VariableAnalysisMap = analysis_variable_analyses_module.VARIABLE_ANALYSES
HIGH_CONFIDENCE_VARIABLE_ANALYSIS_KEYS: tuple[str, ...] = (
    analysis_variable_analyses_module.HIGH_CONFIDENCE_VARIABLE_ANALYSIS_KEYS
)
LOW_CONFIDENCE_VARIABLE_ANALYSIS_KEYS: tuple[str, ...] = (
    analysis_variable_analyses_module.LOW_CONFIDENCE_VARIABLE_ANALYSIS_KEYS
)
emit_output: Callable[..., None] = console_module.print_output  # type: ignore[assignment]

EXIT_SUCCESS: int = _EXIT_SUCCESS
EXIT_FAILURE: int = _EXIT_FAILURE
EXIT_USAGE_ERROR: int = _EXIT_USAGE_ERROR

CONFIG_PATH: Path = _config_module.get_config_path()
DEFAULT_CONFIG = _config_module.DEFAULT_CONFIG

MenuOption = startup_application.MenuOption


TargetLoadError = project_support_module.TargetLoadError


def _print_validation_warnings(warnings: list[str], *, limit: int = 12) -> None:
    project_support_module.print_validation_warnings(warnings, print_fn=print, limit=limit)


def _target_validation_warnings(target_name: str, warnings: list[str]) -> list[str]:
    return project_support_module.target_validation_warnings(target_name, warnings)


def load_config(path: Path) -> LoadedConfig:
    return _config_module.load_config(path)
...
def save_config(path: Path, cfg: ConfigDict) -> None:
    from .cli.config import save_config as _cli_save_config

    _cli_save_config(path, cfg)


def self_check(cfg: ConfigDict) -> bool:
    return _config_module.self_check(cfg)
...
def _clear_windows_console() -> None:
    core_terminal_module.clear_windows_console()


def clear_screen() -> None:
    if has_textual_menu_interaction():
        return
    core_terminal_module.clear_screen(os_module=os, sys_module=sys, clear_windows_console=_clear_windows_console)
...
def pause() -> None:
    ...
    if interaction is not None:
        interaction.pause()
        return
    core_interaction_module.pause()


QuitAppError = core_interaction_module.QuitAppError


def quit_app() -> None:
    core_interaction_module.quit_app(clear_screen_fn=clear_screen)
...
def confirm(msg: str) -> bool:
    ...
    return core_interaction_module.confirm(msg)
...
def prompt(msg: str, default: str | None = None) -> str:
    ...
    return core_interaction_module.prompt(msg, default)


def target_exists(target: str, cfg: ConfigDict) -> bool:
    return _config_module.target_exists(target, cfg)


def apply_debug(cfg: ConfigDict) -> None:
    core_apply_debug(cfg)


def build_cli_parser() -> argparse.ArgumentParser:
    from .cli.entry import build_cli_parser as _build_cli_parser

    return _build_cli_parser()


def run_syntax_check_command(file_path: str, *, output_format: str = "text") -> int:
    return run_syntax_check_command_impl(file_path, output_format=output_format)
...
def set_interactive_ui_mode(ui_mode: str | None) -> None:
    from .cli._interaction import set_interactive_ui_mode as _set

    _set(ui_mode)


def reset_interactive_ui_mode() -> None:
    from .cli._interaction import reset_interactive_ui_mode as _reset

    _reset()


def get_interactive_ui_mode() -> str:
    from .cli._interaction import get_interactive_ui_mode as _get

    return _get()


def set_textual_menu_interaction(interaction: Any) -> None:
    from .cli._interaction import set_textual_menu_interaction as _set

    _set(interaction)


def clear_textual_menu_interaction() -> None:
    from .cli._interaction import clear_textual_menu_interaction as _clear
...
def choose_menu_option(
    ...
    *,
    ...
    return core_interaction_module.choose_menu_option(
        title,
        options,
...
def build_menu_interaction() -> Any:
    ...
    if interaction is not None:
        return interaction
    return cli_interaction_module.build_menu_interaction(
        print_menu_fn=_print_menu,
...
def resolve_interactive_ui_mode(cfg: ConfigDict, override_ui_mode: str | None = None) -> str:
    ...
    from . import ui as app_textual_module
...
_get_analyzed_targets = project_support_module.get_analyzed_targets
_require_analyzed_targets = project_support_module.require_analyzed_targets
_has_analyzed_targets = project_support_module.has_analyzed_targets
_require_targets_for_menu_action = startup_application.require_targets_for_menu_action
_cache_key_for_target = project_application.cache_key_for_target
_split_csv_values = project_support_module.split_csv_values


config_module = _config_module
validate_icf_entries_against_program: Callable[..., Any] = validate_icf_entries_against_program
analyze_variables: Callable[..., Any] = analyze_variables
...
def main(argv: list[str] | None = None) -> int:
    ...
    return cli_startup.main(argv, run_main_loop_fn=run_interactive_session)
