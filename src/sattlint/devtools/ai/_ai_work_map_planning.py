...
from ._ai_work_map_parsing import dict_entries
...
def instruction_lookup(work_map: dict[str, Any]) -> dict[str, JsonDict]:
    ...
    for entry in dict_entries(work_map.get("instructions")):
...
def simplify_check_catalog(catalog: dict[str, Any], *, source: str | None = None) -> list[dict[str, Any]]:
    ...
    for entry in dict_entries(catalog.get("checks")):
...
def all_check_entries(work_map: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for collection_name in ("pipeline_checks", "repo_audit_checks"):
        checks.extend(dict_entries(work_map.get(collection_name)))
    return checks
...
def collect_relevant_checks(work_map: dict[str, Any], recommended_check_ids: list[str]) -> list[JsonDict]:
    checks = [
        *dict_entries(work_map.get("pipeline_checks")),
        *dict_entries(work_map.get("repo_audit_checks")),
    ]
...
def match_owner_suites(
    ...
    *,
    ...
    for plan in dict_entries(work_map.get("owner_suite_plans")):
        for suite in dict_entries(plan.get("suites")):
...
def match_agents(
    ...
    *,
    ...
    for entry in dict_entries(work_map.get("agents")):
        agent_lookup[_string_value(entry.get("name"), "unknown")] = entry
    ranked: list[AgentMatch] = []
    for rule in dict_entries(work_map.get("agent_routing")):
...
def select_finish_gate_template(work_map: dict[str, Any], selected_surface: str) -> FinishGateTemplate | None:
    for entry in dict_entries(work_map.get("finish_gate_templates")):
...
def match_blocking_invariants(
    ...
    *,
    ...
    for entry in dict_entries(work_map.get("blocking_invariant_rules")):
