...
from sattlint.devtools.shared.pipeline_checks import collect_repo_file_inventory, path_matches_globs
...
def _catalog_registry_path(source: str) -> str:
    if source == "pipeline":
        return "src/sattlint/devtools/shared/pipeline_checks.py"
    return "src/sattlint/devtools/audit/repo_audit_entrypoints.py"
...
def verify_ai_harness_freshness(  # noqa: PLR0915
    ...
    *,
    ...
    instructions = dict_entries(resolved_work_map.get("instructions"))
    ...
    for entry in dict_entries(resolved_work_map.get("agents")):
        agent_name = _string_value(entry.get("name")).strip()
        if agent_name:
            agents[agent_name] = entry
    routed_agents: set[str] = set()
    for rule in dict_entries(resolved_work_map.get("agent_routing")):
            ...
            issues.append(
                {
                    ...
                    "path": "src/sattlint/devtools/ai/ai_work_map.py",
    ...
    for collection_name in ("pipeline_checks", "repo_audit_checks"):
        for entry in dict_entries(resolved_work_map.get(collection_name)):
