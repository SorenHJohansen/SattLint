...
@lru_cache(maxsize=8)
def _tracked_path_ancestors(tracked_paths: tuple[str, ...]) -> frozenset[str]:
    ancestors: set[str] = set()
    for path in tracked_paths:
        current = path.strip().replace("\\", "/").rstrip("/")
        while current:
...
def is_ignored_repo_path_reference(candidate: str, tracked_paths: tuple[str, ...] | None) -> bool:
    ...
    if tracked_paths is not None and normalized in _tracked_path_ancestors(tracked_paths):
        return False
