...
def _selected_check_exit_code(summary: dict[str, Any], fail_on: str) -> tuple[int, dict[str, Any]]:
    ...
    selected_status = (
        "fail"
        if summary.get("cli_consistency_status") == "fail" or _should_fail(selected_findings, fail_on)
        else "pass"
    )
    ...
    status_report = {
        ...
        "blocking_finding_count": _blocking_finding_count(_summary_findings(summary), fail_on),
        "fail_on": fail_on,
...
def _emit_status_report(
    ...
    *,
    ...
    if output_format == "json":
        _print_json_payload(status_report)
        return
    print_cli_summary_fn(status_report)


def run_parsed_args(args: argparse.Namespace, *, parser: _ErrorCapableParser | None = None) -> int:
    if parser is None:
        parser = build_cli_parser(prog="sattlint repo-audit")
    repo_audit = _repo_audit_module()
    ...
    if args.recommend_checks:
        ...
        _print_json_payload(_build_recommend_checks_stdout_report(report))
        return 0
    if args.apply_ai_gc:
        report = repo_audit.apply_ai_gc(
            output_dir=Path(args.output_dir).resolve(),
        )
        _print_json_payload(report)
    ...
    if args.planning_context:
        ...
        _print_json_payload(_build_planning_context_stdout_report(report))
        return 0
    if args.check_my_changes:
        ...
        _print_json_payload(_build_check_my_changes_stdout_report(report))
    ...
    if args.check:
        ...
        _emit_status_report(
            ...
            print_cli_summary_fn=repo_audit._print_cli_summary,
        )
        return exit_code
    ...
    status_report = {
        "profile": summary["profile"],
        "overall_status": "fail" if _should_fail(_summary_findings(summary), fail_on) else "pass",
        "findings_schema": summary.get("findings_schema"),
        "finding_count": summary["finding_count"],
        "findings": _terminal_findings(summary),
        "blocking_finding_count": _blocking_finding_count(_summary_findings(summary), fail_on),
    ...
    return 1 if _should_fail(_summary_findings(summary), fail_on) else 0
