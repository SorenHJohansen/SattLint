"""Recommended repo-audit slice execution helpers."""
...
def _recommended_slice_runs_module() -> Any:
    from . import _repo_audit_entrypoint_runs as runs_module  # noqa: PLC0415

    return runs_module


def _recommended_slice_progress_stage_duration(progress: ProgressReporter, key: str) -> float:
...
def _run_recommended_slice_pipeline(
    *,
    output_dir: Path,
    profile: str,
    skip_vulture: bool,
...
def _run_recommended_slice_custom_scan(
    *,
    repo_root: Path,
...
def run_recommended_repo_audit_slice(  # noqa: PLR0915
    ...
    *,
    ...
    entrypoints_module = _recommended_slice_entrypoints_module()
    ...
    with ThreadPoolExecutor(max_workers=RECOMMENDED_REPO_AUDIT_MAX_WORKERS) as executor:
        if pipeline_check_ids:
            pipeline_future = executor.submit(
                _run_recommended_slice_pipeline,
        ...
        if repo_finding_check_ids or "ai-gc" in repo_check_ids or "cli-consistency" in repo_check_ids:
            custom_scan_future = executor.submit(
                _run_recommended_slice_custom_scan,
    ...
    summary = {
        ...
        "timing": {
            "stage_durations_seconds": {
                "pipeline": round(_recommended_slice_progress_stage_duration(progress, "pipeline"), 3),
                "custom_scan": round(_recommended_slice_progress_stage_duration(progress, "custom_scan"), 3),
                "merge_findings": round(_recommended_slice_progress_stage_duration(progress, "merge_findings"), 3),
                "write_reports": 0.0,
            },
...
__all__ = ["run_recommended_repo_audit_slice"]
