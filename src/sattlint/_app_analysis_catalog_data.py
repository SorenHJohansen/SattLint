...
from ._app_analysis_catalog_metadata import analyzer_issue_exclusive_group_id
...
_VARIABLE_DESCRIPTIONS: dict[str, str] = {
    "1": "Run the maintained high-confidence variable report set in one pass.",
    "2": "List declarations that never participate in reads, writes, mappings, or other observable usage.",
    "3": "Show datatype fields that remain unused even when the surrounding record or structure is declared.",
    "4": "Highlight values that are only read so they can be reviewed for CONST promotion or stronger contracts.",
    "5": "Report variables that receive writes but never contribute to later reads or outputs.",
    "6": "Find parameter mappings that reference targets the resolved module interface does not provide.",
    "7": "Find string-like mappings whose source and destination datatypes do not line up safely.",
    "8": "Surface repeated complex datatype layouts that should likely be unified before they drift apart.",
    "9": "Show suspicious Min_/Max_ mapping pairs whose names suggest the wrong signal pairing.",
    "10": "Collect raw literals that would be easier to review and tune as named constants or parameters.",
    "11": "Report required parameter connections that are still missing from the resolved module contract.",
    "12": "Show record layouts whose meaning depends on field order instead of stable field names.",
    "13": "Report reset paths that can leak stale state because expected reset writes are missing.",
    "14": "Report inner declarations that hide another visible variable or module-local name.",
    "15": "Flag variables that appear to exist only to feed UI, HMI, or display-oriented behavior.",
    "16": "Highlight procedure-status patterns that look incomplete, one-sided, or inconsistent across steps.",
    "17": "Show writes that appear to have no downstream consumer or behavioral effect.",
    "18": "Report likely datatype or contract mismatches across module boundaries and mappings.",
    "19": "Highlight values that seem to latch implicitly because the matching clear path never appears.",
    "20": "Find globals that could likely move to a narrower scope without losing required visibility.",
    "21": "Surface shared-state hotspots where hidden global coupling is likely driving behavior.",
    "22": "Show variables with unusually high fan-in or fan-out that deserve architectural review.",
...
SECTION_SPECS: tuple[AnalysisSectionSpec, ...] = (
    ...
    AnalysisSectionSpec(
        ...
        description="Focused variable reports with a relatively low expected false-positive rate.",
        sort_order=300,
    ),
    AnalysisSectionSpec(
        section_id=SECTION_VARIABLE_LOW_CONFIDENCE,
        label="Heuristic reports",
        description="Signal-finding reports that usually need manual confirmation before action.",
        sort_order=400,
    ),
    AnalysisSectionSpec(
    ...
    AnalysisSectionSpec(
        ...
        label="Granular analyzer checks",
        description="Single-select issue slices extracted from bundled registry analyzers.",
        sort_order=950,
    ),
    AnalysisSectionSpec(
        section_id=SECTION_CATALOG_ANALYZERS,
        label="Additional analyzers",
        description="Registry-backed analyzers that are not already covered by the maintained TUI sections.",
        sort_order=1000,
    ),
)
...
TOP_LEVEL_ANALYSIS_FAMILIES: tuple[TopLevelAnalysisFamilySpec, ...] = (
    ...
    TopLevelAnalysisFamilySpec(
        ...
        classic_menu_key="6",
        navigation_mode="submenu",
        section_ids=(SECTION_CATALOG_SUITE, SECTION_CATALOG_ISSUE_CHECKS, SECTION_CATALOG_ANALYZERS),
    ),
    TopLevelAnalysisFamilySpec(
    ...
    entries.extend(
        [
            AnalysisCatalogEntry(
                ...
                description="Trace field-level reads, writes, and mapping usage for one selected variable name.",
                execution=AnalysisExecutionSpec(
                    kind="run_handler",
            ...
            AnalysisCatalogEntry(
                ...
                description="Show where one variable or field is read, written, and referenced across the target.",
                execution=AnalysisExecutionSpec(
                    kind="run_handler",
            ...
            AnalysisCatalogEntry(
                ...
                description="Inspect local-variable and field usage within one resolved module path.",
                execution=AnalysisExecutionSpec(
                    kind="run_handler",
            ...
            AnalysisCatalogEntry(
                ...
                execution=AnalysisExecutionSpec(
                    ...
                    exclusive_group_id=analyzer_issue_exclusive_group_id("mms-interface"),
                    suite_role="suite",
                ),
                sort_order=700,
                classic_menu_key="1",
            ...
            AnalysisCatalogEntry(
                ...
                execution=AnalysisExecutionSpec(
                    ...
                    exclusive_group_id=analyzer_issue_exclusive_group_id("comment-code"),
                    suite_role="suite",
                ),
                sort_order=800,
                classic_menu_key="1",
