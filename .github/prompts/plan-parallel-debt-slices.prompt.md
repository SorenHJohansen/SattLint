...
- For each proposed slice, include the grouped debt IDs, purpose, suggested owner agent, exact file claims, branch name, worktree name, suggested task path, suggested handoff path, first validation command, and blockers or conflicts.
- If two debt IDs want the same controlling file or the same shared AI-control file other than `.github/coordination/current-work.md`, do not mark them parallel-safe until the conflict is isolated or sequenced.
