...
- If the slice claims an oversized structural debt file from `artifacts/analysis/file_debt_ratchet.json`, make the reduction intent explicit in the task metadata and prefer extracting code into sibling modules over appending more code into the debt-heavy owner file.
