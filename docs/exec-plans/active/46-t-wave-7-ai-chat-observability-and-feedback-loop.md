...
- [ ] Implement a repo-owned transcript reader and artifact writer under `src/sattlint/devtools/` that treats transcript JSONL as the source of truth for chat behavior.
- [ ] Add a session-index health probe so the report can explain whether `session_store_sql` is usable, degraded, or empty for the current workspace.
- [ ] Record AI quality findings such as empty assistant output, discovery-before-action counts, failed tool calls, and repeated same-tool retries in a structured findings artifact.
- [ ] Add focused tests and a small fixture transcript corpus so future chat-review work can be validated without reading real machine-local chat logs.
- [ ] Add one documented refresh command or VS Code task entry so the report becomes a repeatable maintenance artifact instead of a one-off debugging script.
...
At creation time, no code has landed yet. The current outcome is a scoped plan that converts a manual transcript review into a repeatable repo artifact and makes AI chat quality measurable.
