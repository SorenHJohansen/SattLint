...
Acceptance is behavioral. After this plan is implemented, a pull request must still trigger the integrated `CI` workflow and the owner workflows, but only one workflow may own the authoritative Ubuntu full repo audit. A `main` push must no longer schedule both `ci.yml` and `repo-audit.yml` to run equivalent full-audit commands on Ubuntu.
