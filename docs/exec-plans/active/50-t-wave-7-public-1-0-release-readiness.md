...
- [x] (2026-05-20) Rewrite the top-level `README.md` away from coworker-only language so it states the public support contract, points users at support and security policy docs, and gives a source-checkout evaluation path that matches the current preview-stage distribution reality.
- [ ] Finish the public-facing top-level docs so an external user can install from a published artifact once available, and so the remaining release-facing docs stop describing aspirational or disconnected behavior as if it were stable.
...
The first implemented slices now define the public support contract in-repo, add missing community-health files, rewrite the top-level README away from coworker-only language, and route GitHub issues through explicit support and security policy documents. Repo-audit also enforces the new public support docs and README links so later release work cannot silently drift back to an undocumented state. The main remaining public-doc gap is not tone or discoverability anymore; it is the absence of a real published-artifact install path and the final stable-release wording that should land only after the release rehearsal exists.
...
The public-facing docs are spread across `README.md`, `CONTRIBUTING.md`, `SECURITY.md`, `CHANGELOG.md`, and `docs/references/cli-commands.md`. Today those files are more trustworthy than they were at plan creation because the README now states the support contract and points to the support and security policy docs directly. The remaining gap is that evaluation still routes through a source checkout and `pipx install .`, not a published artifact path, so the final stable-release doc pass must happen together with the release rehearsal.
