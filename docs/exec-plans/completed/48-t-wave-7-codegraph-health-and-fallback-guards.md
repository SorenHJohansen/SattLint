...
- [x] (2026-05-15) Extend the repo health surface so one command can report CodeGraph health, index presence, MCP wiring, CLI readiness, status-command results, and fallback guidance.
- [x] (2026-05-15) Add fixture-driven tests for healthy, degraded, and fallback-to-rg CodeGraph scenarios in `tests/devtools/test_context_health.py`.
- [x] (2026-05-15) Update the CodeGraph task and top-level guidance so the health check becomes the required first step before CodeGraph exploration.
- [x] (2026-05-15) Update AGENTS and the CodeGraph routing skill so repeated failed MCP calls become a documented anti-pattern rather than a silent fallback.
...
Observation: fixture-backed CodeGraph health testing can stay deterministic without depending on a live MCP session.
Evidence: `tests/devtools/test_context_health.py` now stubs CLI status responses and `.vscode/mcp.json` state directly while exercising healthy, degraded, and fallback routing.
...
The repo-owned health seam now exposes a dedicated CodeGraph section through `python scripts/context_health.py --check --section codegraph`. It distinguishes `healthy`, `degraded`, and `fallback_to_rg` states, reports index and MCP prerequisites, and emits route guidance instead of forcing agents to infer the next step from raw CLI output.

The `AI: CodeGraph Health` task, `AGENTS.md`, the CodeGraph routing skill, and the CodeGraph maintenance prompt now all route through that repo-owned check. Repeated failed MCP calls after a non-healthy health result are now documented as an anti-pattern.
