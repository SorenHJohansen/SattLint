# pyright: reportUnknownVariableType=false, reportUnknownMemberType=false, reportUnknownParameterType=false, reportMissingParameterType=false, reportUnknownArgumentType=false, reportUnusedFunction=false
...
SAMPLE_FIXTURE_DIR = Path(__file__).resolve().parent / "fixtures" / "sample_sattline_files"
...
def test_discover_workspace_sources_finds_programs_and_dependencies(tmp_path):
    ...
    _write_text(tmp_path / "artifacts" / "Generated.s", '"skip"\n')
    _write_text(tmp_path / "node_modules" / "Vendor.s", '"skip"\n')
    ...
    assert tmp_path / "artifacts" / "Generated.s" not in discovery.program_files
    assert tmp_path / "node_modules" / "Vendor.s" not in discovery.program_files
    assert discovery.abb_lib_dir == tmp_path / "ABB_lib"
    assert discovery.referenced_program_names == frozenset({"dep", "vendordep"})
...
def test_load_workspace_snapshot_resolves_definitions_from_graphics_companion(tmp_path):
    source = """
"SyntaxVersion"
"OriginalFileDate"
"ProgramDate"
...
ENDDEF (*BasePicture*);
""".strip()
    graphics = """" Syntax version 2.23, date: 2026-04-22-00:00:00.000 N "
...
"""
    ...
    entry_file = tmp_path / "Program" / "Main.s"
    graphics_file = entry_file.with_suffix(".g")
    _write_text(entry_file, source)
    _write_text(graphics_file, graphics)
    ...
    error_line, error_column = _line_col_for(graphics, "ErrorIndex")
    print_line, print_column = _line_col_for(graphics, "PrintList")

    error_defs = snapshot.find_definitions_at(graphics_file, error_line, error_column)
    print_defs = snapshot.find_definitions_at(graphics_file, print_line, print_column)

    assert len(error_defs) == 1
    assert error_defs[0].canonical_path == "BasePicture.ErrorIndex"
    assert len(print_defs) == 1
    assert print_defs[0].canonical_path == "BasePicture.PrintList"


def test_load_workspace_snapshot_resolves_inline_graphics_bindings_from_fixture() -> None:
    entry_file = SAMPLE_FIXTURE_DIR / "TestGFileParse.s"
    graphics_file = entry_file.with_suffix(".g")
    graphics = graphics_file.read_text(encoding="utf-8")
    ...
    expected_paths = {
        "IndexVar": "BasePicture.IndexVar",
        "LineColorVar": "BasePicture.LineColorVar",
        "index1pathvar": "BasePicture.index1pathvar",
        "DefaultPathVar": "BasePicture.DefaultPathVar",
        "resetvar": "BasePicture.resetvar",
        "minvar": "BasePicture.minvar",
        "maxvar": "BasePicture.maxvar",
        "barvariable": "BasePicture.barvariable",
        "areacolorvar": "BasePicture.areacolorvar",
        "textcolorvar": "BasePicture.textcolorvar",
    }

    for token, canonical_path in expected_paths.items():
        line, column = _line_col_for(graphics, token)
        definitions = snapshot.find_definitions_at(graphics_file, line, column)
        assert len(definitions) == 1
        assert definitions[0].canonical_path == canonical_path
