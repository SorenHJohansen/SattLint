# pyright: reportUnknownVariableType=false, reportUnknownMemberType=false, reportUnknownParameterType=false, reportMissingParameterType=false, reportPrivateUsage=false, reportUnknownArgumentType=false
...
def test_line_findings_skip_repo_audit_self_scan_shards_and_scanner_sources(tmp_path):
    shard_file = tmp_path / "tests" / "test_repo_audit_part1.py"
...
def test_run_text_scan_check_skips_local_ci_parity_findings(tmp_path):
    ...
    findings = repo_audit.run_text_scan_check(context)
...
def test_parse_coverage_findings_handles_missing_file_and_parse_error(tmp_path):
    assert repo_audit.parse_coverage_findings(tmp_path) == []
