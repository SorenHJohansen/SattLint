...
def test_debug_unresolved_lookup_ignores_interact_widget_tokens_and_windowcontent():
    code = """
"SyntaxVersion"
"OriginalFileDate"
"ProgramDate"
...
"""
    ...
    assert analyzer._unresolved_variable_lookup_total == 0
    assert analyzer._unresolved_variable_lookup_counts == {}


def test_debug_unresolved_lookup_ignores_builtin_on_and_off_names():
    bp = BasePicture(
        header=_hdr("BasePicture"),
        datatype_defs=[],
        moduletype_defs=[],
    ...
    analyzer._walk_tail("On", context, [bp.header.name])
    analyzer._walk_tail("Off", context, [bp.header.name])

    assert analyzer._unresolved_variable_lookup_total == 0
    assert analyzer._unresolved_variable_lookup_counts == {}
    assert analyzer._unresolved_variable_lookup_examples == {}
