...
def build_coverage_summary_report(root: Path) -> dict[str, Any]:
    return _repo_audit_module()._coverage_reports_module.build_coverage_summary_report(root)
...
def _parse_coverage_findings(
    ...
    *,
    ...
    findings: list[Any] = []
    root_xml = repo_audit.ElementTree.fromstring(coverage_path.read_text(encoding="utf-8"))
...
def _find_structural_report_findings(root: Path | None = None) -> list[Any]:
    ...
    architecture_report = repo_audit.structural_reports_module.collect_architecture_report(report_root)
