...
def build_pipeline_check_catalog(
    *,
    ...
    return pipeline_module._build_pipeline_check_catalog(
        profile=profile,
...
def _build_recommendation_why_this_gate(
    *,
    ...
    for entry in recommended_checks:
        matched_files = pipeline_module.matching_changed_files(normalized_changed_files, entry["path_globs"])
...
def build_pipeline_check_recommendations(
    *,
    ...
    elif pipeline_module.matching_changed_files(
        resolved_changed_files, pipeline_module.PIPELINE_RECOMMENDATION_FALLBACK_GLOBS
    ):
        fallback_required = True
        fallback_reason = (
    ...
    if fallback_required and fallback_reason is not None:
        ...
        for entry in catalog["checks"]:
            matched_files = pipeline_module.matching_changed_files(resolved_changed_files, entry["path_globs"])
